...
class RenderLess {
    ...
    async renderStyle() {
        const { data, options, mixins } = this;
        ...
        try {
            const out = await less.render(this.source, {
                compress: false,
                modifyVars: this._buildVars(data),
                ...options
            });
        ...
        for (const name in data) {
            ...
            if (deph < 7 && value && typeof value == 'object') {
                ...
                if (!Array.isArray(value) || (value.length && typeof value[0] == 'object')) {
                    lessVars = this._extend(lessVars, this._buildVars(value, fullPath, deph));
                }
                // это массив
                else {
